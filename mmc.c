...
char mmc_buffer[SD_BUFF_SIZE] = { 0 };               // Buffer for mmc i/o for data and registers
...
unsigned long mmcReadCardSize(void)
{
  ...
  if( !response )
  {
    while ((b != 0xFE) && (count++ < 64) ) b = spiSendByte(DUMMY_CHAR);
    // bits 127:87
...
unsigned long mmcReadCardSize1(void)
{
  // Read contents of Card Specific Data (CSD)
 ...
 // response = mmcGetXXResponse(0);
  ...
  //    if(1)
  {
   ...
   // printf("\n\r mmc_C_SIZE %d ", mmc_C_SIZE);
    ...
    b = spiSendByte(DUMMY_CHAR);
    mmc_C_SIZE_MULT += b >> 7;
 //   printf("\n\r mmc_C_SIZE_MULT %d ", mmc_C_SIZE_MULT);
